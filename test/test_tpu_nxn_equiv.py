"""Gate test for src/tpu_nxn.sv (roadmap item 5d-2), N=2 full-chip
equivalence half. Written by the harness author, not the agent - per
tinytpu-loop README "the gate grows with the design".

The dump wrapper (test/dump_tpu_nxn_equiv.sv) instantiates the LEGACY
tpu and the new tpu_nxn side by side, sharing every input. This test
drives both with the COMPLETE test_tpu.py instruction script (forward
pass, transition, backward pass, weight-gradient tiling) and compares
the two UB memory images WORD FOR WORD after a generous drain. The
legacy chip is the spec: any port-wiring or integration mistake in
tpu_nxn shows up as a memory divergence. (Cycle-level timing differs
by one cycle - systolic_nxn registers its left-edge inputs on entry -
so the comparison is on the final architectural state, not waveforms.)

Unlike the upstream tests, the asserts here are LIVE - PYTHONOPTIMIZE
is empty (NOASSERT is never defined), so they fire.
"""
...
async def test_tpu(dut): 
    ...
    dut.ub_rd_row_size.value = 0
    dut.ub_rd_col_size.value = 0
    await FallingEdge(dut.tpu_legacy.vpu_valid_out_1)  # wait until last value of vpu is done
    ...
    await FallingEdge(dut.tpu_legacy.vpu_valid_out_1)
    ...
    await ClockCycles(dut.clk, 40)  # generous drain (nxn is +1 cycle)

    # Compare the two UB memory images word for word. Legacy is the spec.
    mismatches = []
    for a in range(128):
    ...
    assert not mismatches, (
        f"tpu_nxn N=2 UB memory diverges from legacy tpu in "
        f"{len(mismatches)} word(s): " + "; ".join(mismatches[:16]))
    print("tpu_nxn N=2 full-chip equivalence pass OK "
          "(128-word UB image identical after forward+backward+gradients)")
